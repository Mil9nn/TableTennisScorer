...
interface JoinShareActionsProps {
  tournamentName: string;
  joinCode: string;
}

export function JoinShareActions({ tournamentName, joinCode }: JoinShareActionsProps) {
  ...
  const shareInvite = async () => {
    try {
      await Share.share({
        message: buildShareMessage({ tournamentName, joinCode: normalized }),
        title: `Join ${tournamentName}`,
        url: joinLink,
      });
    } catch (err) {
      console.error("Error sharing tournament invite:", err);
    }
  };
