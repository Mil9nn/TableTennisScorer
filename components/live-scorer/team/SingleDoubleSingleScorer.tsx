...
const getSubMatchLabel = (index: number): string => {
...
export default function SingleDoubleSingleScorer({
  ...
  const player1 = Array.isArray(player1Raw) ? player1Raw : [player1Raw];
  const player2 = Array.isArray(player2Raw) ? player2Raw : [player2Raw];

  const teamMatchPlayers = {
    side1: player1.map((p: any) => ({
      name: p?.fullName || p?.username || "Player",
      playerId: p?._id,
      serverKey: "team1",
      profileImage: p?.profileImage,
    })),
    side2: player2.map((p: any) => ({
      name: p?.fullName || p?.username || "Player",
      playerId: p?._id,
      serverKey: "team2",
      profileImage: p?.profileImage,
  ...
  const player1Names = teamMatchPlayers.side1.map((p) => p.name).join(" & ");
  const player2Names = teamMatchPlayers.side2.map((p) => p.name).join(" & ");
          ...
          <View style={styles.navigatorHeader}>
            ...
            <View style={styles.badge}>
              <Text style={styles.badgeText}>
                {getSubMatchLabel(currentSubMatchIndex)} · Match{" "}
                {currentSubMatchIndex + 1} of {activeMatch.subMatches.length}
              </Text>
            </View>
          </View>
